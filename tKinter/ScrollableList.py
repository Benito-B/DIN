...
another_frame = ttk.Frame(root, padding=50)
another_frame.grid(column=0, row=0, sticky="NSWE")
c = ttk.Frame(another_frame, padding=(10, 10, 12, 8), relief=SUNKEN)
...
# Scrollbar needs the method of the listbox yview and the listbox needs the yscrollcommand of the scrollbar
scrollbar = ttk.Scrollbar(c, orient=VERTICAL, command=lbox.yview)
lbox['yscrollcommand'] = scrollbar.set
# Sizegrip for increasing the inner frame
ttk.Sizegrip(root).grid(column=4, row=7, sticky="SE")
...
lbox.grid(column=1, row=0, rowspan=6, sticky=(N, S, E, W))
...
c.grid_columnconfigure(1, minsize=120, weight=1)
